...
| any      | analysis          | llvm           | 14      | `clang-tidy` with `-bugprone-exception-escape` mishandles `std::string` from MSVC STL and flags all usage of potentially throwing rendering this check useless.       | **None.** Do not use for now.                                                                                                          | 02/10/2021 |
