...
TEST_CASE("adb::insert().node() -> Query [adb::Query]")
{
    const auto query = adb::insert()
                           .node();
    REQUIRE(std::get<adb::Query::InsertNodes>(query.data()).values.size() == 1);
}
...
TEST_CASE("adb::insert().nodes(acore::size_type) -> Query [adb::Query]")
{
    const auto query = adb::insert()
                           .nodes(3);
    REQUIRE(std::get<adb::Query::InsertNodes>(query.data()).values.size() == 3);
}
...
TEST_CASE("adb::insert().nodes(std::vector<std::vector<adb::KeyValue>> values) -> Query [adb::Query]")
{
    const auto query = adb::insert()
                           .nodes({{{"Key1", "Value1"}, {"Key2", 4}},
                                   {{"Key1", "Value2"}},
                                   {{1, 2}, {-3, -4}}});
    REQUIRE(std::get<adb::Query::InsertNodes>(query.data()).values
