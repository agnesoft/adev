...
static const auto testSuite = suite("abuild::Project", [] {
    test("type traits", [] {
        ...
        expect(std::is_copy_constructible_v<abuild::Project>).toBe(false);
        expect(std::is_nothrow_move_constructible_v<abuild::Project>).toBe(true);
        expect(std::is_copy_assignable_v<abuild::Project>).toBe(false);
    ...
    test("name", [] {
        ...
        expect(project.name()).toBe(std::string{"myproject"});
    });
});
