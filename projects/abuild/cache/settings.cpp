...
export struct Settings
{
    ...
    //! compiled directly.
    std::vector<std::string> cppHeaderExtensions = {".hpp", ".hxx", ".h"};

    //! Extensions of files that will be
    //! considered sources and that will be
    //! compiled directly.
    std::vector<std::string> cppSourceExtensions = {".cpp", ".cxx", ".cc", ".ixx"};

    //! Names of source files (without extension)
    //! that indicates the containing project is
    //! to be an executable.
    std::vector<std::string> executableFilenames = {"main", "Main", "WinMain"};

    //! Directories to ignore.
    std::vector<std::string> ignoreDirectories = {"build"};

    //! Directories not to consider as project
    //! directories. Does not affect
    //! subdirectories that will be scanned
    //! normally.
    std::vector<std::string> skipDirectories = {"projects", "Projects"};

    //! Directories considered part of the
    //! containing project. E.g. `my_project` ->
    //! `my_project`, `my_project/src` ->
    //! `my_project`.
    std::vector<std::string> squashDirectories = {"src", "srcs", "SRC", "Src", "source", "sources", "Source", "Sources", "include", "Include", "includes", "Includes"};

    //! Directores considered as containing test
    //! project of the containing project.
    std::vector<std::string> testDirectories = {"test", "Test", "tests", "Tests"};
};
}
