...
static const auto S = suite("import include external", [] { // NOLINT(cert-err58-cpp)
    test("normal import", [] {
        const std::vector<::abuild::Token> tokens = ::abuild::tokenize("import <header.hpp>;");

        assert_(tokens.size()).to_be(1U);
        assert_(std::holds_alternative<::abuild::ImportIncludeExternalToken>(tokens[0])).to_be(true);

        const auto &token = std::get<::abuild::ImportIncludeExternalToken>(tokens[0]);
    ...
    test("missing opening bracket with another import", [] {
